    ...
    public partial class Gambling : DiscordModule
    {
        ...
        public async Task BetRoll(long amount)
        {
            ...
            if (rng < 67)
            {
                ...
                await Context.Channel.SendErrorAsync(str).ConfigureAwait(false);
            }
            else
            {
                if (rng < 91)
                {
                    ...
                    await CurrencyHandler.AddCurrencyAsync(Context.User, "Betroll Gamble", (long)Math.Floor(amount * 1.5), false).ConfigureAwait(false);
                }
                else if (rng < 100)
                {
                    str += $"Congratulations! You won {amount * 5}{Gambling.CurrencySign} for rolling above 90.";
                    await CurrencyHandler.AddCurrencyAsync(Context.User, "Betroll Gamble", amount * 5, false).ConfigureAwait(false);
                }
                else
                {
