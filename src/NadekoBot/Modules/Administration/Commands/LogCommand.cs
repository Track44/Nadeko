        ...
        public class LogCommands : ModuleBase
        {
            ...
            private static async void MuteCommands_UserMuted(IGuildUser usr, MuteCommands.MuteType muteType)
            {
                try
                {
                    ...
                    await logChannel.EmbedAsync(new EmbedBuilder().WithAuthor(eab => eab.WithName("🔇 User Muted from " + mutes))
            ...
            private static async void MuteCommands_UserUnmuted(IGuildUser usr, MuteCommands.MuteType muteType)
            {
                try
                {
                    ...
                    await logChannel.EmbedAsync(new EmbedBuilder().WithAuthor(eab => eab.WithName("🔊 User Unmuted from " + mutes))
                            .WithTitle($"{usr.Username}#{usr.Discriminator} | {usr.Id}")
                            .WithFooter(fb => fb.WithText($"{currentTime}"))
                            .WithOkColor()).ConfigureAwait(false);
                }
                ...
                try
                {
                    ...
                    if (before.Nickname != after.Nickname)
                    {
                        embed.WithAuthor(eab => eab.WithName("👥 Nickname Changed"))
                            ...
                            .AddField(efb => efb.WithName("Old Nickname").WithValue($"{before.Nickname}"))
                            .AddField(efb => efb.WithName("New Nickname").WithValue($"{after.Nickname}"));
                    }
